...
\noindent where, $g(\cdot)$ is a real-value differentiable and invertible link function and $\boldsymbol{\beta}$ is a vector of $p$ regression parameters. $\{U(\mathbf{s}): \mathbf{s} \in D\}$ is stationary spatial Gaussian process with zero mean and spatial covariance function $\mathsf{Cov}(U(\mathbf{s}),U(\mathbf{s}')) = C(\mathbf{s} - \mathbf{s}'; \boldsymbol{\theta})$

where $\boldsymbol{\theta}$ is a vector of correlation parameters. $\mathbf{u} = (u(s_1),u(s_2),\ldots,u(s_n))^{\top}$ is a realization of $U(\cdot)$. Conditionally on $\mathbf{u}$, $\mathbf{y} = (y(s_1),y(s_2),\ldots,y(s_n))^{\top}$ are mutually independent.

The form of conditional density function of $y_i = y(s_i)$ given $u_i = u(s_i), i = 1, \ldots, n$ is
$$
f(y_i|u_i;\boldsymbol{\beta}) = \exp[a(\mu_i)y_i - b(\mu_i)]c(y_i)
$$ 
