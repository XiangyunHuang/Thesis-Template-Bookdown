...
其中二次有理型自相关函数 $\rho(u) = (u/\phi)^2/[1 + (u/\phi)^2],u \leq \phi$，则半变差函数 $V(u) = 1-\rho(u) = [\tau + (u/\phi)^2]/[1 + (u/\phi)^2]$。当距离 $u = \phi$ 时，变差等于 $(1+\tau)/2$，由图\@ref(fig:yields-variogram)可知 $\tau = 0.2$， 样本变差就等于 0.6 对应的距离，大约是 13，所以 $\phi=13$。
