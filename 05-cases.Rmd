...
蒙特卡罗极大似然算法迭代的初值 $\beta_{0} = 6.2,\sigma^2_{0} = 2.40,\phi_{0} = 340,\tau^2_{rel} =2.074$，模拟次数为 30000 次，前 10000 次迭代视为预热阶段 (warm-up) 而不用于参数的估计，其后每隔 20 次迭代采一个样本点，存储参数的迭代值。
...
Table: (\#tab:rangelap-mcml-result) 蒙特卡罗极大似然估计模型 \@ref(eq:rongelap-with-nugget-effect) 的参数

| 参数        |   $\hat{\beta}$ | $\hat{\sigma}^{2}$  |   $\hat{\phi}$  | $\hat{\tau}^2$    |  $\log L_{m}$    |
| :---------- | :-------------  | :-------------      | :------------   | :---------------  |:---------------: |
|   初始值    |    6.2          |     2.40            |   340           |   2.074           |         -        |
|   估计值    |    6.190        |     2.401           |   338.126       |   2.053           |    0.002892      |
