...
```{r rongelap-trace-plot,fig.cap="Langevin-Hastings 算法模拟条件分布 $[S(x\\_{i})|Y\\_{i}], i = 1,\\ldots,4$，第一列是迭代序列图，第二列是对应的密度分布"}
knitr::include_graphics(path = "figures/rongelap-mcml-diagnosis-trace-9.png")
```
从图 \@ref(fig:rongelap-trace-plot) 可以看出迭代序列符合平稳性的特征

```{r rongelap-acf-plot,fig.cap="条件分布 $[S(x\\_{i})|Y\\_{i}], i = 1,\\ldots,4$ 的采样序列的自相关图"}
