...
# 一维指数型
cat(readLines("code/stan/05-sim_one_gp_exp.stan"), sep = "\n")
# 一维幂二次指数型
# cat(readLines("code/stan/05-sim_one_gp_exp_quad.stan"), sep = "\n")
```
...
## 交互图  {#Interactive-Web-Maps .unnumbered}


```{r rongelap-leaflet,fig.cap="朗格拉普岛上核污染分布图",eval=!is_on_travis,echo=FALSE,message=FALSE,out.width='100%'}
invisible(
  lapply(c("geoRglm", "sp", "rgdal", "leaflet"), function(pkg) {
...
spst <- spTransform(sps, CRS("+proj=longlat +ellps=clrk66"))
...
```{r compiling-sampling,fig.cap="基于 Stan 编程的模型编译和采样过程图",fig.subcap=c("编译","采样"), eval=!is_on_travis,echo=FALSE}
...
```{r,eval=!is_on_travis,echo=!is_on_travis}
