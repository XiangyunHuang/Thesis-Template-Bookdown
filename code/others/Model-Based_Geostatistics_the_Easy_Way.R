...
rainCol <- colourScale(swissRain$rain,
  col = "Blues", dec = -1,
...
plot(plotRaster,
  alpha = 0.8, add = TRUE, col = brewer.pal(8, "Greens"),
...
plot(plotRaster,
  add = TRUE, col = brewer.pal(8, "Greens"),
...
legend("bottomright",
  pch = 15, pt.cex = 2.5, col = c(rev(mycol), NA), legend = rev(mybreaks),
  adj = c(0, -0.5)
)
#'
...
rcl <- rbind(
  c(9, 8), # savannas to woody savannas
...
#+ loaFit, cache=TRUE
loaFit <- glgm(
  formula = y ~ land + evi + elHigh + elLow,
  data = loaloa, grid = 50,
  covariates = covList,
...
map.new(loaloa)
plot(africaTiles, add = TRUE)
plot(loaFit$raster[["random.mean"]],
  col = thebreaks$col, breaks = thebreaks$breaks,
...
legend("bottomright",
  pch = 15, pt.cex = 2, col = c(NA, rev(thecol)),
...
countcol <- colourScale(kentucky$observed,
  breaks =
...
expcol <- colourScale(kentucky$expected,
  breaks =
...
legend("topleft",
  pch = 15, pt.cex = 2.5, col = c(NA, rev(substr(coloursExc, 1, 7))),
...
values(covariates)[, 2] <- rep(
  seq(0, 1, len = nrow(covariates)),
...
myModel <- c(
  intercept = 0.5, variance = 2^2, nugget = 0.5^2, range = 2.5, shape = 2,
  cov1 = 0.2, cov2 = -0.5
)
library("abind")
Npoints <- 50
