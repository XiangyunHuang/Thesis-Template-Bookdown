...
// Fixed kernel hyperparameters: phi = 0.2, sigma^2 = 0.5, kappa = 0.5, alpha = 0

data {
  int<lower=1> N;
  matrix[N, N] x;
  int<lower=1> COVFN;
  int<lower=0,upper=4> y[N];
}
transformed data {
  real delta = 1e-12;
}
...
model {
  ...
  phi ~ inv_gamma(5, 5);
  sigmasq ~ scaled_inv_chi_square(5, 0.5);
  alpha ~ normal(0, 1);
  eta ~ normal(0, 1);

  y ~ binomial_logit(4, alpha + f);
}
