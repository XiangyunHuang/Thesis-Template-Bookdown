...
\noindent 其中，$\beta_0$ 是截距，$\beta_{1},\beta_{2}$ 是固定效应 $X_{1}$ 和 $X_{2}$ 的系数，$S(x)$ 是平稳空间高斯过程，其均值为 $\mathbf{0}$，协方差函数为 $\mathrm{Cov}(S(x_i),S(x_j)) = \sigma^2 \big\{2^{\kappa-1}\Gamma(\kappa)\big\}^{-1}(u/\phi)^{\kappa}K_{\kappa}(u/\phi)$，在后续的模拟中 $\kappa = 0.5$，$g$ 是联系函数，由响应变量 $Y$ 服从的分布决定，在第 \@ref(binomal-sglmm) 小节中，$g(\mu_i) = \log\{\frac{p(x_i)}{1-p(x_i)}\}$，$Y_{i} \sim \mathrm{Binomal}(m_{i},p(x_{i}))$，在位置 $x_i$ 处，以概率 $p(x_i)$ 重复抽取了 $m_i$ 个样本，总样本数 $M=\sum_{i=1}^{N}m_i$，$N$ 是采样点的个数，在第 \@ref(possion-sglmm) 小节中，$g(\mu_i) = \log\{\lambda(x_{i})\}$，$Y_i \sim \mathrm{Possion}(\lambda(x_{i}))$。
