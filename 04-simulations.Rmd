...
(ref:one-dim-gp) 模拟一维平稳空间高斯过程，协方差函数分别为指数型 \@ref(eq:cov-exp) 和幂二次指数型 \@ref(eq:cov-exp-quad)，均值为 $\mathbf{0}$，协方差参数 $\sigma^2 = 1$，$\phi = 0.15$，横坐标表示采样的位置，纵坐标是目标值 $S(x)$，图中 2000 个灰色点表示服从对应随机过程的随机数，橘黄色点是从中随机选择的 36 个点。
...
二维情形下，在规则平面上模拟平稳高斯过程 $\mathcal{S} = S(x), x \in \mathbb{R}^2$， 其均值向量为零向量 $\mathbf{0}$， 协方差函数为指数型 (见公式 \@ref(eq:cov-exp)) ，协方差参数 $\phi = 1, \sigma^2 = 1$。在单位平面区域为 $[0,1] \times [0,1]$ 模拟服从上述二维空间平稳高斯过程，不妨将此区域划分为 $6 \times 6$ 的小网格，每个格点作为采样的位置，而在这些格点上的观察值即为目标值 $S(x)$。 
...
```{r sim-two-gp,fig.cap="模拟二维平稳空间高斯过程，自相关函数为指数形式，水平方向为横坐标，垂直方向为纵坐标，图中的格点是目标值 $S(x)$", small.mar=TRUE,fig.asp=1,out.width="55%",fig.width=4.5}
set.seed(2018)
N <- 36
phi <- 1
sigma <- 1
...
同 \@ref(sim-one-gp) 节，平稳空间高斯过程 $S(x)$ 的协方差函数也可以为更一般的梅隆型，如公式 \@ref(eq:exp-matern) 所示。

\begin{equation}
\rho(u) = \{ 2^{\kappa -1} \Gamma(\kappa) \}^{-1}( u/\phi )^{\kappa} \mathcal{K}_{\kappa}( u/\phi ) (\#eq:exp-matern)
\end{equation}

\noindent 且在区域 $[0,1] \times [0,1]$ 上也可以随机采点，图中 $S(x)$ 的值越大，颜色越红，反之，颜色越蓝。
