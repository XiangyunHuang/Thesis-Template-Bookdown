...
基于 Stan 实现的贝叶斯 STAN-MCMC 算法，其内置的 HMC 算法是结合了 NUTS 采样器，搜索模型参数的策略更加友好，不需要手动调参数，只需要指定合适的参数先验，使得迭代序列保持收敛即可，编程过程中，对模型参数的重参数化对迭代进程和结果会产生一定影响，如第\@ref(prepare)章第\@ref(sec:stan-samplers)节基于 Eight Schools 数据集介绍分层正态模型就对参数 $\mu$ 和 $\sigma$ 做了重参数化(reparameterization)。

基于似然推断的算法，如第\@ref(algorithms)章第\@ref(subsec:MCML)小节介绍的蒙特卡罗极大似然算法和第\@ref(subsec:LA)小节介绍的拉普拉斯近似算法，都需要非常接近真值的参数初值，才能得到好的结果，因为在大多数情形下，SGLMM 模型的对数似然曲面是呈现山岭或峡谷状，局部极值点多而且对数似然函数值变化不大，导致收敛速度极慢甚至不收敛，非常难得到全局极大。因此，一个合适的策略是在合理的初值周围打网格，格点作为迭代初值，以成千上万个不同初值进行迭代，将计算的对数似然值画在二维平面上，通过这种降维观察的方式，获得一个可靠的全局极大值点，作为参数的最佳似然估计，在后续的第\@ref(applications)章第\@ref(case-rongelap)节以分析朗格拉普岛核污染数据集为例，介绍这一策略。
