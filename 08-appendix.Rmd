...
Table: (\#tab:LH-binom-SGLMM) Langevin-Hastings 算法：模型 \@ref(eq:binom-SGLMM) 中 64 个采样点处 $p(x_i) = \exp[S(x_i)]/\{1 + \exp[S(x_i)]\}$ 的后验分布的均值(mean)，方差(variance)，标准差(standard deviation)和 5 个分位点，样本量为 1000 
...
## 模拟数据 {#simulate-sglmm .unnumbered}
...
```{r compiling-sampling,fig.cap="基于 Stan 编程的模型编译和采样过程",fig.subcap=c("编译","采样"), eval=is_on_travis,echo=is_on_travis}
