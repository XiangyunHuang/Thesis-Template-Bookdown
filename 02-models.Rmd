...
```
```{r matern-3d,fig.cap="自相关函数 $\\rho(u)$ 在不同空间距离$\\mathsf{u}$处随贝塞尔函数的阶 $\\kappa$ 和尺度参数 $\\phi$ 的变化，横轴表示尺度参数 $\\phi$，纵轴表示贝塞尔函数的阶$\\kappa$，我们用 从蓝到红的颜色变化表示自相关函数 $\\rho(u)$ 值由小到大，即表示相关性由弱变强",out.width="80%"}
