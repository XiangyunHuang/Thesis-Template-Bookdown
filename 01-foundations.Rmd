...
设 $x_{1}, \ldots, x_{n}$ 是来自概率密度分布 $p(x;\theta)$ 的一个样本，叙述简单起见，考虑单参数情形，参数空间 $\boldsymbol{\Theta}$ 是一个开区间，$l(\theta;x) = \sum_{i=1}^{n}\ln p(x_{i};\theta)$。
...
```{theorem, label="asymptotic-normality", name="渐进正态性", echo=TRUE}

```


```{theorem, label="asymptotic-effectiveness", name="渐进有效性", echo=TRUE}
