...
|                   |      正态分布      |      泊松分布      |      二项分布      |
| :---------------- | :----------------: | :----------------: | :----------------: | 
|  记号             | $\mathcal{N}(\mu,\sigma^2)$  |       $P(\mu)$     |     $B(m,\pi)/m$   |
...
在给定先验分布 $\pi(\theta)$ 和平方损失 $L(\theta,\delta) = (\delta - \theta)^2$ 下，$\theta$ 的贝叶斯估计 $\delta^{\pi}(x)$ 为后验分布 $\pi(\theta|x)$ 的均值，即 $\delta^{\pi}(x) = E(\theta|x)$。
```
...
当 $\epsilon$ 较小时，$\theta$ 的贝叶斯估计为后验分布 $\pi(\theta|x)$ 的众数。
```
