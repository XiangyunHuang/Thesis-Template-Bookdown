...
空间过程的协方差函数是梅隆型时，需要用到修正的第二类贝塞尔函数 $\mathcal{K}_{\kappa}(u)$，它是修正的贝塞尔方程的解 [@Abramowitz1972]，函数形式如下
...
其中 $u \geq 0$，$\kappa \in \mathbb{R}$，如果 $\kappa \in \mathbb{Z}$，则取该点的极限值，$\mathcal{K}_{\kappa}(u)$ 的值可由 R 内置的函数 `besselK` 计算 [@Campbell1980]。
...
在获得样本 $Y$ 后，模型参数 $\boldsymbol{\theta}$ 的后验分布 (posterior distribution) 就是在给定 $Y=y$ 条件下 $\boldsymbol{\theta}$ 的条件分布。
