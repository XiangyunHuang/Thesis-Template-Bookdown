...
```{r yields-variogram,fig.cap="空间变差"}
knitr::include_graphics(path = "figures/Yields-Variogram.png")
```

我们使用球形结构拟合这组数据中的空间结构，位置参数 $\phi$ 和块金效应的初始值可以从图 \@ref(fig:yields-variogram) 获取，其中 $\phi$ 是样本变差等于 1 时对应的空间距离。所以拟合模型 \@ref(eq:extended-linear-model)
