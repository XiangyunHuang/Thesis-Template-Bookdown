...
|          |  mean  |   MCSE  | StdDev|  2.5% |   25% |   50% |   75% | 97.5% | n_eff | Rhat |
|  :-----  |  ----: |   ----: | -----:| ----: | ----: | ----: | ----: | ----: | ----: | ----:|       
...
表 \@ref(tab:eight-schools-output) 的列依次是后验均值的估计、蒙特卡罗误差、后验标准差的估计、后验的5个分位点，余下的有效样本数和潜在规模缩减因子 (potential scale reduction factor) 用来分析采样及其效率；最后一行表示对数后验 (log-posterior) $\hat{R}$，当链条都收敛到同一平稳分布的时候，$\hat{R}$ 接近 1。
