...
Andrew Gelman 和 John B. Carlin 等 (2003年) [@Gelman2003] 建立分层正态模型分析数据集 8schools，这里再次以该数据集和模型为例介绍 Stan 的使用和算法实现，分层正态模型模型结构如下
...
模型参数主要设置了4条链，每条链迭代 10000 次，为复现模型结果随机数种子设为 2018
...
Table: (\#tab:eight-schools-output) 8 schools 分层正态模型参数的估计值
...
表 \@ref(tab:eight-schools-output) 的列为后验量的估计值：依次是均值 $\mathsf{E}(\mu|Y)$、 标准误(standard error) $\mathsf{Var}(\mu|Y)$、标准差 (standard deviation) $\mathsf{E}(\sigma|Y)$ 和后验分布的 5 个分位点，有效样本数 $n_{eff}$ 和潜在尺度缩减因子 (potential scale reduction factor) 用来分析采样及其效率；最后一行表示每次迭代的未正则的对数后验密度 (unnormalized log-posterior density) $\hat{R}$，当链条都收敛到同一平稳分布的时候，$\hat{R}$ 接近 1。


## 潜在尺度缩减因子

为了评估链条之间和内部的混合效果，我们引入潜在尺度缩减因子 $\hat{R}$ [@Gelman2013R]，对每个待估的量 $\omega$，模拟产生 $m$ 条链，每条链有 $n$ 次迭代值 $\omega_{ij} (i = 1,\ldots,n;j=1,\ldots,m)$，用 $B$ 和 $W$ 表示链条之间（不妨叫组间方差）和内部的方差（组内方差）
...
\begin{equation}
\hat{\mathsf{Var}}^{+}(\omega|Y) = \frac{n-1}{n} W + \frac{1}{n} B 
\end{equation}
...
\begin{equation}
\hat{R} = \sqrt{\frac{\hat{\mathsf{Var}}^{+}(\omega|Y)}{W}}
\end{equation}

随着 $n \to \infty$， $\hat{R}$ 下降到 1，如果 $\hat{R}$ 比较大，我们有理由认为需要增加模拟次数，改进待估参数 $\omega$ 的后验分布。
