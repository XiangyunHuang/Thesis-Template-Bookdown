...
plot(Variogram(m1, form = ~ latitude + longitude, data = Wheat2))
...
一般地，我们说样本 $\mathbf{Y}$ 的分布服从指数族，是指分布可以写成 [@McCullagh1989]
...
通过与 \@ref(eq:common-exponential-family) 式对比，可知 $\theta = \mu$，$\phi = \sigma^2$，
...
可见，$Y$ 的方差是两个函数的乘积，一个是 $b''(\theta)$， 它仅仅依赖典则参数，被叫做方差函数，另一个是 $a(\phi)$，它独立于 $\theta$，仅仅依赖 $\phi$，方差函数可以看作是 $\mu$ 的函数，记作 $V(\mu)$。根据 \@ref(eq:common-exponential-family)式，正态、泊松和二项分布的特征见表 \@ref(tab:common-characteristics)，其它常见分布见 Peter McCullagh 等 （1989年） [@McCullagh1989]。
...
| 典则联系：$\theta(\mu)$   |  identity |    log      |     logit      |
...
为什么不用 RMSE 均方误差，WAIC pDIC 模型选择 loo K-CV  
